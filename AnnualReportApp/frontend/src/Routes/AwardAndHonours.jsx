...
const AwardAndHonours = (props) => {
    const setUser = props.setUser
    const type = ['Award', 'Honour']
    const editMode = 'edit'
    const deleteMode = 'delete'
    const [Award_or_honour,setType] = useState()
    const [award_name,setAward] = useState()
    const [award_by,setAwardBy] = useState()
    const [award_for,setAwardFor] = useState()
    ...
    useEffect(()=>{
        fetchData.post('user/get/data/award_and_honours',{email}).then((res) => {
            setLoading(false)
    ...
    const checkStates = () => {
        const states = [
            Award_or_honour,award_name,award_by,award_for,email,
                name,date
        ]
    ...
    const uploadData = (e) => {
        ...
        setErr()
        if(checkStates()){
        const body = {Award_or_honour,award_name,award_by,award_for,email,sorting_no,
        name,timestamp: new Date(date).getTime(),date}
        setLoading(true)

        fetchData.post('user/upload/data/award_and_honours',body).then((res)=>{
            setLoading(false)
    ...
    const Records = ({id, x}) => {
        ...
        const deleteOrUpdateContent = () => {
            ...
            if(mode === editMode) {
                ...
                const fields = ['Award_or_honour', 'award_name','award_by', 'award_for']
                ...
                setLoading(true)
                fetchData.post('user/update/data/award_and_honours', {...req_body,date,_id: x._id }).then((res) => {
                ...
                fetchData.delete(`user/delete/award_and_honours/${x._id}`).then((res) => {
        ...
        const cancel = () => {
            const data = [x.Award_or_honour, x.award_name, x.award_by,x.award_for]
            const div_grps = document.getElementsByClassName(`input-div${id}`)
            setDate(x.date)
            for (let i = 0; i < div_grps.length; i++) {
        ...
        return (
            ...
            <div contentEditable={mode === editMode} className={`flex1 input-div${id}`}>{x.Award_or_honour}</div>
            <div contentEditable={mode === editMode} className={`flex1 input-div${id}`}>{x.award_name}</div>
            <input style={color} type='date' className={`flex1 unstyled date-picker${id}`} 
            onChange={e => setDate(e.target.value)} value={date} />
            <div contentEditable={mode === editMode} className={`flex1 input-div${id}`}>{x.award_by}</div>
            <div className="flex1">
                <div contentEditable={mode === editMode} className={`last-col input-div${id}`}>{x.award_for}</div>
                { mode ? <>
        ...
        return (
            <div className="entries-div">
                <div className="entries-row header">
                    ...
                    <div className="flex1">Award/Honour</div>
                    <div className="flex1">Award/Honour Name</div>
                    <div className="flex1">Date</div>
                    <div className="flex1">Award by</div>
                    <div className="flex1">Award for</div>
                </div>
                    ...
                    {err ? <p style={{color: 'crimson',marginBottom: '10px',textAlign:'center'}}> {err} </p> : <> </>}
                        ...
                        <div className="row">
                            ...
                            <label className='form-label' htmlFor="text">
                                Award or Honour<span style={{ color: 'red' }}>*</span>
                            </label>
                            <select className="form-input" value={Award_or_honour}
                            ...
                            <div className='right'>
                            <label className='form-label' htmlFor="text">Award/Honour Name<span style={{ color: 'red' }}>*</span></label>
                            <input className='form-input' type="text" value={award_name} 
                            onChange={e=>setAward(e.target.value)} />
                            </div>
                        </div>
                        <div className="row">
                        <div className='left'>
                            <label className='form-label'>Awarded by<span style={{ color: 'red' }}>*</span></label>
                            <input className='form-input' type="text" value={award_by} 
                            onChange={e=>setAwardBy(e.target.value)}/>
                        </div>
                        <div className='right'>
                            <label className='form-label'>Awarded for<span style={{ color: 'red' }}>*</span></label>
                            <input className='form-input' type="text" value={award_for} 
                            onChange={e=>setAwardFor(e.target.value)}/>                        
                            </div>
                        </div>
                        <div className="row">
                        <div className='left'>
                            <label className='form-label' htmlFor="text">Date<span style={{ color: 'red' }}>*</span></label>
...
export default AwardAndHonours