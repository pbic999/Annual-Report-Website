...
const SpecialLectures = (props) => {
    ...
    useEffect(()=>{
        fetchData.post('user/get/data/special_lecture',{email}).then((res) => {
            setLoading(false)
    ...
    const uploadData = (e) => {
        ...
        setErr()
        if(checkStates()) {
        const body = {topic,institute,email,sorting_no,
        name,timestamp: new Date(date).getTime(),date}
        setLoading(true)
        fetchData.post('user/upload/data/special_lecture',body).then((res)=>{
         ...
         setErr('Please fill all mandetory fields.')
    }
    ...
    const Records = ({id,x}) => {
        ...
        const deleteOrUpdateContent = () => {
            ...
            if(mode === editMode) {
                ...
                setLoading(true)
                fetchData.post('user/update/data/special_lecture', {...req_body,date,_id: x._id }).then((res) => {
                ...
                fetchData.delete(`user/delete/special_lecture/${x._id}`).then((res) => {
        ...
        const cancel = () => {
            const data = [x.topic, x.institute]
            const div_grps = document.getElementsByClassName(`input-div${id}`)
            setDate(x.date)
            for (let i = 0; i < div_grps.length; i++) {
        ...
        return (
            ...
            <div contentEditable={mode === editMode} className={`flex1 input-div${id}`}>{x.topic}</div>
        ...
        return (
            <div className="entries-div">
                <div className="entries-row header">
                    ...
                    <div className="flex1">Lecture Topic</div>
                    <div className="flex1">Institution</div>
                    ...
                    {err ? <p style={{color: 'crimson',marginBottom: '10px',textAlign:'center'}}> {err} </p> : <> </>}
                        ...
                        <div className="row">
                            ...
                            <label className='form-label' htmlFor="text">
                                Lecture Topic<span style={{ color: 'red' }}>*</span>
                            </label>
                            <input className='form-input' type="text" value={topic}
                            onChange={e => setTopic(e.target.value)} />
                            </div>
                            <div className='right'>
                            <label className='form-label' htmlFor="text">Institute<span style={{ color: 'red' }}>*</span></label>
                            <input className='form-input' type="text" value={institute}
